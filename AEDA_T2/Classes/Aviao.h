...
class Aviao{
    ...
    int custo_operacao;
    vector<Localizacao*> codigoAeroporto;
    int tempTotal;
    int tempMedio;
...
public:
    ...
    Aviao(string tipo, int capacidade, vector<Voo*> voos, vector<Funcionario*> funcionarios, int custo_operacao, int tempTotal, int tempMedio)
    {
        this->tipo = tipo;
    ...
    /// Calcula tempo total gasto dos avioes na pista
    void CalculaTempoTotal()
    {
        for(int i= 0; i < voos.size(); i++)
    ...
    /// Calcula tempo medio dos avioes na pista
    void CalculaTempoMedio()
    {
        tempMedio = tempTotal/(voos.size());
    }

    /// Adiciona codigos dos aeroportos que o aviao passou
    void AdicionaCodigoAeroporto(Localizacao *l)
    {
        for(int i=0; i < codigoAeroporto.size(); i++)
            codigoAeroporto.push_back(l);
    }
    ...
    int getTempoTotal(){return tempTotal;}
