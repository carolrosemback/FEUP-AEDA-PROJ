...
Aeroporto::Aeroporto(Funcionario *ger, Localizacao loc, vector<Funcionario *> fu, vector<Aviao *> av,
                     vector<Piloto *> pil, vector<Membro_tripulacao *> mem,
                     vector<Funcionario_administrativos *> f_adm){
    gerente = ger;
    localizacao = loc;
    funcionarios = fu;
    avioes = av;
...
void Aeroporto::CalculaTempoMedio()
{
    int n = 0;
    for (int i = 0; i < avioes.size(); i++) {
...
int Aeroporto::getTempoMedio() const {
